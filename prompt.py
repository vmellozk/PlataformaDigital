def get_initial_prompt():
    return (f'Olá! Você está prestes a criar um eBook para um cliente. Você receberá instruções para gerar um eBook completo e detalhado, seguindo um padrão específico. '
            f'Primeiro, você irá receber os prompts, e depois vou mandar as respostas. Com base nessas respostas, eu vou te pedir para criar o ebook no padrão fornecido. '
            ...
            f'####Capa: (vou pedir de forma separada), ####Sumário: serão 10 tópicos onde serão listados os assuntos que serão abordados ao longo do ebook. Os tópicos serão de sua escolha, mas tem que ter relação com as respostas. '
            ...
            f'Primeiro eu vou passar as respostas, mas NÃO RESPONDA! Vou te pedir o título separado do conteúdo. Espere o pedido.')

def responses(responses_text):
    return (f"{responses_text}")

def tittle(name):
    return(f'"#### Capa: Título e junto com o nome do autor que é: {name}". O título será de sua escolha. Lembrando da hash antes. Apenas responda o que foi pedido, sem "essa foi a resposta, se precisar de mais..." não quero nada disso. ')
