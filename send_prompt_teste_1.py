...
def send_prompts(input_field, responses_file, tittle_file, name):
    ...
    # Aguarda a resposta ser gerada e o botão de descer para o final estar disponível
    while True:
        try:
            button_location = pyautogui.locateCenterOnScreen('static/error/button_end.png')
            if button_location:
                pyautogui.click(button_location)
                time.sleep(5)
                break
            else:
                print("Botão de descer não encontrado.")
                time.sleep(1)
        except Exception as e:
            print(f"Erro durante a automação: {e}")
            time.sleep(1)
