...
def process_user(user_id):
    ...
    try:
        ...
        semaphore.release()  # Libera o semáforo após o processamento
...
def process_tabs():
    with ThreadPoolExecutor(max_workers=MAX_TABS) as executor:
        ...
        while True:
            ...
            futures.append(executor.submit(process_user, user_id))
            # Espera que pelo menos um futuro seja concluído antes de verificar a fila novamente
            ...
            if not tab_queue.full():
                if user_id not in tab_queue.queue:
                    ...
                    if semaphore.acquire(blocking=False):  # Tenta adquirir o semáforo
                        threading.Thread(target=process_tabs).start()
                else:
